...
export async function POST(request) {
  try {
    ...
    const tempDir = path.join(os.tmpdir(), `spotliner-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    ...
    let filePathForDb;
    const ext = path.extname(downloadedPath) || ".mp3";
    const uniqueName = `${Date.now()}-${Math.random().toString(36).slice(2)}${ext}`;

    if (process.env.BLOB_READ_WRITE_TOKEN) {
      const bytes = fs.readFileSync(downloadedPath);
      const blob = await put(`audio/${uniqueName}`, new Blob([bytes], { type: "audio/mpeg" }), {
        access: "public",
